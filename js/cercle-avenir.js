  ...
  lesConteneurMask[i].addEventListener("mouseover", function () {
    ...
    verification = setTimeout(function () {
      if (hover) {
        ...
        leTexte.style.transform =
          "translate(" +
          (window.innerWidth / 2 -
            leTexte.offsetLeft -
            leTexte.offsetWidth / 2) +
          "px," +
          (window.innerHeight / 2 - leTexte.offsetTop) +
          "px) scale(3)"; // Déplace le texte
...
for (let i = 0; i < lesConteneurMask.length; i++) {
  lesConteneurMask[i].addEventListener("mouseout", function () {
    ...
    leTexte.style.transform =
      "translate(" +
      (lesOffsetLeft[i] - leTexte.offsetLeft) +
      "px," +
      (lesOffsetTop[i] - leTexte.offsetTop) +
      "px) scale(1)"; // Réinitialise la position
