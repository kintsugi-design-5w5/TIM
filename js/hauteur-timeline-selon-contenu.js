...
function mettreAJourCercles() {
  ...
  cercles.forEach((cercle) => {
    ...
    if (
      window.scrollY >= seuilActivation &&
      !cercle.classList.contains("cercle-actif")
    ) {
...
document.addEventListener("click", () => {
  setTimeout(() => {
    if (
      localStorage.getItem("rest-cours") !== null &&
      localStorage.getItem("rest-cours") === "true"
    ) {
      mettreAJourCercles();
      gestionDescriptions();
...
function gestionDescriptions() {
  ...
  document.querySelectorAll(".cours-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      console.log("Id description", descriptionElement);
  ...
  document.querySelectorAll(".cercle").forEach((cercle) => {
    cercle.addEventListener("click", (event) =>
      changerIconeCercle(event.target)
    );
  });
}
